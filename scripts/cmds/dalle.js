...
const randomCookie = cookies[Math.floor(Math.random() * cookies.length)];
      const wait = api.sendMessage("Piw Piw Chat Bot creating please w8 🦆", event.threadID);
...
const imageUrls = response.data.imgUrls || [];
   ...
   api.sendMessage({ body: `🎖️ | Piw Piw Chat Bot Here's Your Generated Photo 🦆`, attachment: images }, event.threadID, event.messageID);
    } catch (error) {
      console.error(error);
